...
A flow duration curve (FDC) shows what fraction of time discharge equals or exceeds a given value. `calc_exceedance_prob()` implements the Weibull plotting-position formula. Setting `remove_zeros = TRUE` excludes days with zero discharge from the ranking, so the curve reflects the non-zero portion of the flow regime.
