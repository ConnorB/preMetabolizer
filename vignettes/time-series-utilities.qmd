...
For multi-site data, provide the site column so each site is completed independently.
...
`convert_UTC_to_solartime()` and `convert_solartime_to_UTC()` move between UTC and local solar time at a site longitude. Mean solar time is the time basis expected by stream metabolism models.
...
`get_season()` classifies dates into fixed northern-hemisphere astronomical seasons.
