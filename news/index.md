...
- [`get_ks_meso()`](https://connorb.github.io/preMetabolizer/reference/get_ks_meso.md)
  now correctly reports invalid variable names in the error message
...
- [`get_nasa_data()`](https://connorb.github.io/preMetabolizer/reference/get_nasa_data.md)
  ...
  ranges from the data, uses either single-site coordinate arguments or
  per-site `lat`, `lon`, and `elev_m` columns, interpolates NASA values
  to the input timestamps, and returns `light.obs` by converting
  `ALLSKY_SFC_SW_DWN` with
  [`streamMetabolizer::convert_SW_to_PAR()`](https://rdrr.io/pkg/streamMetabolizer/man/convert_SW_to_PAR.html)
  (#TBD).
